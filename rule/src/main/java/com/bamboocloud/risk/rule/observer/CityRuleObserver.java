...
public class CityRuleObserver extends AbstractRuleObserver {

    @Override
    protected String getType() {
        return RuleType.CITY.name();
    }

    @Override
    protected boolean isHit(Rule rule, UserRuleInfo userRuleInfo) {
        return new UserGeneralInfoRule().hit(rule,userRuleInfo.getId(),userRuleInfo.getCity());
    }
}
